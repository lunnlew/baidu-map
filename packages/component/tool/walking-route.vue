...
const bm = ref<BaiduMapVue3.BMapGL.WalkingRoute | null>()
...
watch(
    ...
    val => {
        ...
        if (val) {
            // 自定义选择路线结果
            merge_props.onSearchComplete = async function (e: BaiduMapVue3.BMapGL.WalkingRouteResult) {
            ...
            bm.value = bindEvents(addWalkingRoute(currentMap.value, merge_props), extractEmitEvents(attrs), emit)
            emit('ready', {
                bmobj: bm.value,
                search: () => {
